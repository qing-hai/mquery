...
//add by qinghai, mongodb 4.0
Query.prototype.expr = function (expression) {
  this._conditions.$expr = expression;
  return this;
}

//add by qinghai, mongodb 4.0
Query.prototype.jsonSchema = function (expression) {
  this._conditions.$jsonSchema = expression;
  return this;
}

//add by qinghai, mongodb 4.0
Query.prototype.text = function (expression) {
  this._conditions.$text = expression;
  return this;
}
...
'gt gte lt lte ne in nin all regex size maxDistance minDistance not type bitsAllSet bitsAnySet bitsAllClear bitsAnyClear'.split(' ').forEach(function ($conditional) {
  Query.prototype[$conditional] = function () {
    ...
    var conds = this._getCondition(path);
...
Query.prototype.select = function select() {
  ...
  if (arg === "*") return this;//support select("*")
  ...
  if (('string' == type || utils.isArgumentsObject(arg)) &&
    ...
    if ('string' == type) {
      var oldArg = arg;
      arg = arg.split(/\,/).map(function (it) { return it.trim() });

      if (arg.length < 2) {
        arg = oldArg.split(/\s+/).map(function (it) { return it.trim() });
      }
    }
...
//add by qinghai
Query.prototype._getCondition = function (path) {
  var rst = this._conditions[path];
  if (rst === undefined) {
    rst = {};
  }
...
//hook Query.prototype, add querybuilder support

Query.supportQueryBuilder=function(obj){
  Object.getOwnPropertyNames(obj).filter(it => it !== "constructor" && ((typeof obj[it]) === "function")).forEach(it => {
    var old = obj[it];
    obj[it] = function () {
      if ('undefined' !== typeof _transformQuery) {
        var args = _transformQuery && Object.values(arguments).map(function (it) {
          if (utils.isArray(it)) {
            it = it.map(function(x) {
              return _transformQuery(x)
            });
  
            return it;
          } else {
            return _transformQuery(it);
          }
        });
 ...
 */
//add by qinghai
//Query.supportQueryBuilder(Query.prototype);
...
//add by qinghai
Query.qb=require("./querybuilder")
var _transformQuery=Query.qb.transformQuery;
